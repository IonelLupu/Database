...
use PDO;
use Closure;
use Webtron\Database\Query\StatementFactory;
...
class Query{
	...
	protected function addParam($value,$type = PDO::PARAM_STR){
		...
		if( !Builder::$paramBinding )
			return Builder::$DB->quote($value);
	...
	public function newQuery( Closure $callback ){
		...
		call_user_func($callback, $query = new Query( Builder::getTable($this->table) ) );
